...
def find_product_by_excel_row(row: dict, products: list):
    ...
    """
    products_found = find_product_by_article(row.get('Артикул', ''), products)
    if products_found:
        return products_found[0]  # Возвращаем первый найденный товар
    products_found = find_product_by_article(row.get('Название', ''), products)
    if products_found:
        return products_found[0]  # Возвращаем первый найденный товар
    return None
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                products_found = find_product_by_article(row_dict['Артикул'], products)
                if not products_found:
                    products_found = find_product_by_article(row_dict['Название'], products)
                if products_found:
